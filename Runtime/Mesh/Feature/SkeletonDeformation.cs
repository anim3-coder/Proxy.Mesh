...
namespace Proxy.Mesh
{
    ...
    public class SkeletonDeformation : IProxyAbstractChild, IProxyJob,IDisposable
    {
        ...
        public JobHandle StartJob(JobHandle dependsOn)
        {
            if (IsInit)
            {
                if(proxy.skeleton.rootBone != null)
                    return CalculateSkeletonDeformation(dependsOn);
                else
                {
        ...
        public JobHandle CalculateSkeletonDeformation(JobHandle dependsOn)
        {
            switch (Method)
            {
                default:
        ...
        public JobHandle CalculateSkeletonDeformationFromGroup(int groupIndex, JobHandle dependsOn)
        {
            return new WSkeletonDeformationJob
            {
                indices = proxy.skeletonGroups.indices[groupIndex],
                baseVertices = proxy.nativeBaseVertices,
                baseNormals = proxy.nativeBaseNormals,
                allBoneWeights = proxy.allBoneWeights,
                bonesPerVertex = proxy.bonesPerVertex,
        ...
        [BurstCompile]
        protected struct WSkeletonDeformationJob : IJob
        {
            ...
            [WriteOnly, NativeDisableParallelForRestriction, NativeDisableContainerSafetyRestriction] public NativeArray<float3> animatedVertices;
            [WriteOnly, NativeDisableParallelForRestriction, NativeDisableContainerSafetyRestriction] public NativeArray<float3> animatedNormals;
            ...
            public void Execute()
            {
                for (int index = 0; index < indices.Length; index++)
                {
                    ...
                    animatedNormal = math.normalize(animatedNormal);

                    animatedVertices[vertexIndex] = animatedVertex;
                    animatedNormals[vertexIndex] = animatedNormal;
                }
            }
        }
